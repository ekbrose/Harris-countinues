...
async def scrape_foreclosures_async(ctx, days_back=45):
    ...
    try:
        ...
        url = f"https://www.cclerk.hctx.net/Applications/WebSearch/FR.aspx"
        ...
        logger.info(f"FRCL: title='{title}' content_len={len(content)}")

        # Try to fill the date form using more generic selectors
        inputs = await page.locator("input[type='text']").all()
        logger.info(f"FRCL: found {len(inputs)} text inputs")
        ...
        if len(inputs) >= 2:
            ...
            logger.info(f"FRCL: found {len(btns)} buttons")
            for btn in btns:
                try:
                    val = await btn.get_attribute("value") or await btn.inner_text()
                    logger.info(f"FRCL button: '{val}'")
                    if re.search(r"search|find|submit|go", val, re.I):
        ...
        logger.info(f"FRCL: {len(rows)} data rows found")
        logger.info(f"FRCL sample row: {rows[0] if rows else 'none'}")
...
async def scrape_tax_async(ctx):
    ...
    try:
        ...
        if rows: logger.info(f"TAX sample: {rows[0]}")
        ...
        for texts in rows:
            ...
            try:
                ...
                amt = re.sub(r"[^\d.]","",amount_raw)
                tax_amt = f"${float(amt):,.0f} owed" if amt else amount_raw
