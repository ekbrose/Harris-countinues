...
async def run_full_scrape_async():
    ...
    try:
        ...
        logger.info("Starting foreclosure scrape...")
        fc_leads = await scrape_foreclosures_async(context, days_back=45)
        logger.info(f"Foreclosure done: {len(fc_leads)} leads")

        logger.info("Starting tax delinquent scrape...")
        tax_leads = await scrape_tax_async(context)
        logger.info(f"Tax done: {len(tax_leads)} leads")

        logger.info("Starting probate scrape...")
        probate_leads = await scrape_probate_async(context, days_back=30)
        logger.info(f"Probate done: {len(probate_leads)} leads")
